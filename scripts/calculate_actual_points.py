...
def find_player_points(df, player_name):
    ...
    # Try partial match
    partial_match = df[
        df["PLAYER"].str.contains(player_name.split()[0], case=False, na=False)
    ]
...
def main():
    ...
    if later_total > earlier_total:
        print(
            f"✅ Later strategy is better by {later_total - earlier_total:.2f} points"
        )
    else:
        print(
            f"❌ Earlier strategy is better by {earlier_total - later_total:.2f} points"
        )
