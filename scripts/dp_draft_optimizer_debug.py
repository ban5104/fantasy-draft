...
def load_and_merge_data() -> List[Player]:
    ...
    if not os.path.exists(espn_file):
        ...
        print("Please ensure the data directory contains the ESPN projections CSV file.")
        sys.exit(1)
    ...
    if low_quality_matches:
        ...
        for match in low_quality_matches[:10]:  # Show first 10 to avoid spam
            print(f"  ESPN: '{match['espn_name']}' -> Best: '{match['best_match']}' ({match['match_score']}%)")
        if len(low_quality_matches) > 10:
            print(f"  ... and {len(low_quality_matches) - 10} more. Consider manual data review.")
...
def export_mc_results_to_csv(players, survival_probs, snake_picks, num_sims):
    ...
    def create_player_row(player):
        row = {'player_name': player.name, 'position': player.position, 
               'fantasy_points': player.points, 'overall_rank': player.overall_rank}
        row.update({f'survival_pick_{pick}': survival_probs.get((player.name, pick), 0.0) 
                   for pick in snake_picks})
        return row
    
    # 1. Player survival data
    player_data = [create_player_row(p) for p in players]
    os.makedirs('../data/output-simulations', exist_ok=True)
    pd.DataFrame(player_data).to_csv('../data/output-simulations/mc_player_survivals.csv', index=False)
        ...
        for pick in snake_picks:
            ...
            if survivals:
                position_data.append({
                    'position': pos, 'pick': pick,
                    'avg_survival_top10': np.mean(survivals),
                    'max_survival_top10': np.max(survivals),
                    'min_survival_top10': np.min(survivals)
                })
    
    pd.DataFrame(position_data).to_csv('../data/output-simulations/mc_position_summary.csv', index=False)
    
    # 3. Configuration metadata
    config_data = {'num_simulations': [num_sims], 'snake_picks': [str(snake_picks)],
                   'position_limits': [str(POSITION_LIMITS)], 'export_timestamp': [datetime.now().isoformat()],
                   'total_players': [len(players)]}
    pd.DataFrame(config_data).to_csv('../data/output-simulations/mc_config.csv', index=False)
    
    print(f"\nExported Monte Carlo results to CSV files:")
    print(f"  - ../data/output-simulations/mc_player_survivals.csv ({len(player_data)} players)")
    print(f"  - ../data/output-simulations/mc_position_summary.csv ({len(position_data)} position/pick combinations)")
    print(f"  - ../data/output-simulations/mc_config.csv (simulation metadata)")
    print(f"\nLoad in Jupyter with: pd.read_csv('../data/output-simulations/mc_player_survivals.csv')")

def monte_carlo_survival_realistic(players: List[Player], num_sims: int, export_simulation_data: bool = False) -> Dict[Tuple[str, int], float]:
    """
                ...
                if scores:
                    ...
                    if export_simulation_data:
                        simulation_picks.append({
                            'simulation': sim, 'pick_number': pick_num,
                            'player_name': picked_player.name, 'position': picked_player.position,
    ...
    if export_simulation_data and simulation_picks:
        import pandas as pd
        pd.DataFrame(simulation_picks).to_csv('../data/output-simulations/mc_simulation_picks.csv', index=False)
        print(f"Exported {len(simulation_picks)} individual simulation picks to ../data/output-simulations/mc_simulation_picks.csv")
...
def ladder_ev_debug(position: str, pick_number: int, slot: int, players: List[Player], 
    ...
    for j in range(slot - 1, min(len(pos_players), survival.shape[0], 10)):
        ...
        if contribution > 0.01 or j < slot + 2:
            debug_info.append(f"    {player.name}: {player.points:.1f}pts × {surv_prob:.2f}surv × {taken_prob:.2f}gone = {contribution:.1f}")
...
def dp_optimize(pick_idx: int, rb_count: int, wr_count: int, qb_count: int, te_count: int) -> Tuple[float, str]:
    ...
    if any(count < 0 or count > POSITION_LIMITS.get(pos, 0) for pos, count in counts.items()):
        return 0.0, ""
    ...
    for pos in ['RB', 'WR', 'QB', 'TE']:
        if counts[pos] < POSITION_LIMITS[pos]:
            ...
            try:
                ev, debug_lines = ladder_ev_debug(pos, current_pick, slot, PLAYERS, SURVIVAL_PROBS)
                
                new_counts = counts.copy()
                new_counts[pos] += 1
                
                future_value, _ = dp_optimize(
                    pick_idx + 1, new_counts['RB'], new_counts['WR'], 
                    new_counts['QB'], new_counts['TE']
                )
                ...
                print(f"ERROR: Exception in DP optimization for {pos} at pick {current_pick}: {e}")
                continue
...
def show_top_players_survival():
    ...
    for pos in ['RB', 'WR', 'QB', 'TE']:
        ...
        for i, player in enumerate(pos_players):
            ...
            surv_str = " ".join([
                f"P{pick}:{survival_matrix[i, pick]:.0%}" if pick < survival_matrix.shape[1] else f"P{pick}:N/A"
                for pick in SNAKE_PICKS
            ])
...
def main():
    ...
    parser = argparse.ArgumentParser(description='DP Draft Optimizer with Monte Carlo Simulation')
    ...
    parser.add_argument('--visualize', action='store_true', help='Generate Monte Carlo visualization dashboard')
    parser.add_argument('--save-plots', action='store_true', help='Save visualization plots as PNG files')
    parser.add_argument('--export-csv', action='store_true', help='Export Monte Carlo results to CSV files')
    parser.add_argument('--export-simulations', action='store_true', help='Export individual simulation pick data for scatter plots')
    
    # Simulation tuning parameters (override config file values)
    parser.add_argument('--randomness', type=float, help='Randomness level 0.0-1.0 (override config)')
    parser.add_argument('--pool-size', type=int, help='Candidate pool size (override config)')
    ...
    print(f"{'='*60}")
    print(f"Randomness Level: {RANDOMNESS_LEVEL} ({'Very Low' if RANDOMNESS_LEVEL <= 0.15 else 'Low' if RANDOMNESS_LEVEL <= 0.25 else 'Moderate' if RANDOMNESS_LEVEL <= 0.4 else 'High' if RANDOMNESS_LEVEL <= 0.6 else 'Very High'})")
    ...
    player_survival = monte_carlo_survival_realistic(players, args.sims, export_simulation_data=args.export_simulations)
    ...
    for pick_idx in range(len(SNAKE_PICKS)):
        ...
        value, position = dp_optimize(pick_idx, counts['RB'], counts['WR'], counts['QB'], counts['TE'])
    ...
    for i, (pick, position) in enumerate(zip(SNAKE_PICKS, sequence)):
        if position:
            ...
            player_info = f" (likely: {likely_player.name})" if likely_player and best_availability > 0.01 else " (no clear favorite)"
